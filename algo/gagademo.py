...
# fret length
# thres or continuous function / weight function adjust
...
def fingraw(fing):
    ...
    #     return score
    return score-2**max(valid_fingers)*1000
...
def fing2score(fing):
    ...
    score = fingraw(fing)
    if fing in statfing:  # past data
        return fingraw(fing)*10000000000+occur[statfing[fing]]
    return fingraw(fing)*10000000000

def fings2score(fing1,fing2):   ## add to two consider 0
    vfing1 = [f for f in fing1 if f >= 0]
    vfing2 = [f for f in fing2 if f >= 0]
    if (len(vfing1)!=1 or len(vfing2)!=1):
        return 0
    # assert(fing1 in statfing and fing2 in statfing)
    wid=abs(fing1.index(vfing1[0])-fing2.index(vfing2[0]))*FWIDTH
    score=-round((wid**2+(FLENGTH[vfing1[0]]-FLENGTH[vfing2[0]])**2)**0.5 / 40 )
    if (fing1 in statfing and fing2 in statfing):
        if statfing[fing2] in trans[statfing[fing1]]:
            score+=round(trans[statfing[fing1]][statfing[fing2]]*10)
    return score/400
...
def solve(file):
    ...
    #     print(notes_seq[t],file=sys.stderr)
    ...
    for t in range(len(times)):
        if len(fings[t])==0:
            dp[t].append(-1e50)
            fr[t].append(0)
            fings[t].append((-1,-1,-1,-1,-1,-1))
            if t==0:
                continue
            for i in range(len(fings[t-1])):
                if dp[t-1][i]>dp[t][0]:
                    dp[t][0]=dp[t-1][i]
                    fr[t][0]=i
            continue
        for i in range(len(fings[t])):
            dp[t][i]=fing2score(fings[t][i])
            if t==0:
                continue
            mxsc=-1e50
            pos=-1
            assert(len(fings[t-1]))
            for j in range(len(fings[t-1])):
                crsc=fings2score(fings[t-1][j],fings[t][i])+dp[t-1][j]
                if crsc>mxsc: ##one
                    mxsc=crsc
                    pos=j
            dp[t][i]+=mxsc
