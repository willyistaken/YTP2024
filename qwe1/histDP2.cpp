	...
	for (int i = 1; i < (int)events.size(); ++i) {
		...
		if (!mp.count(events[i])) {
			...
			for (int j = 1; j < (int)dp[lst].size(); ++j)
				if (dp[lst][j][0] > dp[lst][id][0] || dp[lst][j][0] == dp[lst][id][0] && combCnt[mp[events[lst]][j]] > combCnt[mp[events[lst]][id]])
					id = j;
		...
		for (int j = 0; j < (int)mp[events[i]].size(); ++j) {
			...
			for (int k = 0; k < (int)dp[lst].size(); ++k) {
				...
				if (tmp[0] < val || tmp[0] == val && combCnt[mp[events[lst]][k]] > combCnt[mp[events[lst]][tmp[2]]])
					tmp = {val, lst, k};
			}
